...
If the ML API is unavailable, the backend uses a transparent rule-based fallback. This fallback is not a real credit model. It exists only so reviewers can test the full user-to-admin workflow without starting the Python ML service.
